 ...
 */
import { EDUCATION_RANK } from './enums';
/** 由生日算周岁 */
export function calcAge(birthday, at = new Date()) {
...
export function ageRangeToBirthdayRange(ageMin, ageMax, at = new Date()) {
    const out = {};
...
export function educationRank(e) {
 ...
 */
export function gaussianScore(diff, tolerance) {
    if (tolerance <= 0)
        return diff === 0 ? 1 : 0;
...
export function buildSerialNo(prefix, seq, at = new Date()) {
...
/** 次日零点，权益按日重置用 */
export function startOfNextDay(d = new Date()) {
