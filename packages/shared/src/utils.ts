...
/**
 * 十二生肖，按地支顺序。索引 = (公历年 - 4) % 12。
 * 4 是基准：公元 4 年是鼠年，验证：2020 → (2020-4)%12 = 0 → 鼠 ✓
 */
const ZODIAC = [
  { key: 'rat', label: '鼠', emoji: '🐭' },
  { key: 'ox', label: '牛', emoji: '🐮' },
  { key: 'tiger', label: '虎', emoji: '🐯' },
  { key: 'rabbit', label: '兔', emoji: '🐰' },
  { key: 'dragon', label: '龙', emoji: '🐲' },
  { key: 'snake', label: '蛇', emoji: '🐍' },
  { key: 'horse', label: '马', emoji: '🐴' },
  { key: 'goat', label: '羊', emoji: '🐑' },
  { key: 'monkey', label: '猴', emoji: '🐵' },
  { key: 'rooster', label: '鸡', emoji: '🐔' },
 ...
 * 按出生年取生肖。
 *
 * 注意：生肖实际以农历春节为界，公历 1、2 月出生的人可能属上一个生肖。
 * 这里只按公历年算——我们的存量数据本来就只有出生年，没有月日可依据，
 * 与其用一个假的月日去算"更精确"的结果，不如按年算并接受这个已知偏差。
 */
export function zodiacOf(year: number | null | undefined): Zodiac | null {
  if (!year || !Number.isFinite(year) || year < 1900 || year > 2200) return null;
