 ...
 */
export function resolveViewerLevel(ctx: ViewerContext): VisibilityLevel {
  if (ctx.isAdmin) return VisibilityLevel.ADMIN;
  if (ctx.isSelf) return VisibilityLevel.ADMIN; // 本人看自己等于全可见
...
export function canSee(viewerLevel: VisibilityLevel, fieldLevel: VisibilityLevel): boolean {
 ...
 */
export function maskOrg(org: string | null | undefined): string {
  if (!org) return '';
  const s = org.trim();
...
export const LOCKED_PLACEHOLDER: Record<VisibilityLevel, string> = {
...
export function lockedValue(level: VisibilityLevel, preview?: string): MaskedValue {
  return {
    locked: true,
    requiredLevel: level,
...
export function isMaskedValue(v: unknown): v is MaskedValue {
  return typeof v === 'object' && v !== null && (v as MaskedValue).locked === true;
}
