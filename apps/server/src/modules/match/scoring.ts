...
export function evaluateSatisfaction(
  ...
  if (pref.ageMin != null || pref.ageMax != null) {
    ...
    if (age >= lo && age <= hi) {
      ...
      const s = gaussianScore(over, 3); // 超出 3 岁约得 0.6 分
      parts.push(s);
  ...
  if ((pref.heightMin != null || pref.heightMax != null) && candidate.heightCm != null) {
    ...
    if (h >= lo && h <= hi) {
      ...
      parts.push(gaussianScore(over, 5));
  ...
  if (pref.educationMin) {
    const need = educationRank(pref.educationMin);
    const has = educationRank(candidate.education);
    if (has === 0) {
      parts.push(NEUTRAL);
    } else if (has >= need) {
      parts.push(1);
  ...
  if (pref.incomeMin != null && pref.incomeMin > 0) {
    if (candidate.annualIncome == null) {
      ...
      // 达成率，最低给 0.1
      const ratio = clamp01(candidate.annualIncome / pref.incomeMin);
      parts.push(Math.max(ratio * 0.8, 0.1));
      unmet.push(
        `年收入 ${Math.round(candidate.annualIncome / 10000)} 万，期望 ${Math.round(pref.incomeMin / 10000)} 万以上`,
      );
    }
  }
  ...
  if (pref.maritalStatus.length) {
    if (pref.maritalStatus.includes(candidate.maritalStatus)) {
      parts.push(1);
      met.push(`婚史${MARITAL_LABEL[candidate.maritalStatus]}，符合要求`);
    } else {
      parts.push(0);
      unmet.push(`婚史${MARITAL_LABEL[candidate.maritalStatus]}，对方仅接受${pref.maritalStatus.map((m) => MARITAL_LABEL[m]).join('/')}`);
    }
  }
  ...
  if (pref.requireHouse) {
    const ok = candidate.houseStatus === HouseStatus.MORTGAGE || candidate.houseStatus === HouseStatus.FULL_PAID;
    parts.push(ok ? 1 : 0);
    (ok ? met : unmet).push(ok ? '有房，符合要求' : '对方要求有房');
  }
  if (pref.requireCar) {
    const ok = candidate.carStatus === CarStatus.MORTGAGE || candidate.carStatus === CarStatus.FULL_PAID;
    parts.push(ok ? 1 : 0);
    (ok ? met : unmet).push(ok ? '有车，符合要求' : '对方要求有车');
  }

  if (!parts.length) return { score: 1, met, unmet, criteriaCount: 0 };

  return {
    score: clamp01(parts.reduce((s, v) => s + v, 0) / parts.length),
    met,
    unmet,
...
function scoreAgeGap(a: ScoringProfile, b: ScoringProfile): { raw: number; note: string } {
  ...
  return { raw: gaussianScore(gap, 5), note: `年龄差 ${gap} 岁（${ageA} / ${ageB}）` };
}

function scoreEducation(a: ScoringProfile, b: ScoringProfile): { raw: number; note: string } {
  const ra = educationRank(a.education);
  const rb = educationRank(b.education);
  if (ra === 0 || rb === 0) return { raw: NEUTRAL, note: '学历信息不完整' };
  const gap = Math.abs(ra - rb);
  return { raw: gaussianScore(gap, 1.2), note: gap === 0 ? '学历相同' : `学历相差 ${gap} 档` };
}

function scoreIncome(a: ScoringProfile, b: ScoringProfile): { raw: number; note: string } {
  ...
  if (hi === 0) return { raw: NEUTRAL, note: '收入信息不完整' };
  // 比值越接近 1 越搭。差 3 倍以上明显不搭。
  const ratio = lo / hi;
  return {
    raw: clamp01(ratio ** 0.5),
    note: `年收入 ${Math.round(a.annualIncome / 10000)} 万 / ${Math.round(b.annualIncome / 10000)} 万`,
  };
}

function scoreDistance(a: ScoringProfile, b: ScoringProfile): { raw: number; note: string } {
  ...
  if (a.lat != null && a.lng != null && b.lat != null && b.lng != null) {
    ...
    return { raw: gaussianScore(km, 150), note: `相距约 ${Math.round(km)} 公里` };
  }
 ...
 */
function scoreHeight(a: ScoringProfile, b: ScoringProfile): { raw: number; note: string } {
  if (a.heightCm == null || b.heightCm == null) {
    return { raw: NEUTRAL, note: '身高信息不完整' };
  }
  const male = a.gender === Gender.MALE ? a : b;
  const female = a.gender === Gender.MALE ? b : a;
  if (male.heightCm == null || female.heightCm == null) {
    return { raw: NEUTRAL, note: '身高信息不完整' };
  }
  const diff = male.heightCm - female.heightCm;
  // 理想差 12cm，容忍度 10
  const raw = gaussianScore(Math.abs(diff - 12), 10);
...
export interface MatchScoreInput {
  self: ScoringProfile;
  candidate: ScoringProfile;
  /** L3 语义相似度 0~1，没算时传 null（会用中性分且不计权重） */
...
export function computeMatchScore(input: MatchScoreInput): MatchScoreOutput {
  const { self, candidate, semantic } = input;
  const w = { ...DEFAULT_MATCH_WEIGHTS, ...input.weights };
  ...
  const mutual = harmonicMean(
    selfSatisfiesCandidatePref.score,
    candidateSatisfiesSelfPref.score,
  );

  const items: { key: MatchWeightKey; raw: number; note: string }[] = [
    {
      key: 'mutualPreference',
      raw: mutual,
      note:
        `我方满足对方要求 ${(selfSatisfiesCandidatePref.score * 100).toFixed(0)}%，` +
